...
- [`10.1`, `10.1.7` (*10.1/Dockerfile*)](https://github.com/gitlab-ce-zh/gitlab-ce-zh/blob/master/10.1/Dockerfile)
- [`10.2`, `10.2.7` (*10.2/Dockerfile*)](https://github.com/gitlab-ce-zh/gitlab-ce-zh/blob/master/10.2/Dockerfile)
- [`10.3`, `10.3.6` (*10.3/Dockerfile*)](https://github.com/gitlab-ce-zh/gitlab-ce-zh/blob/master/10.3/Dockerfile)
- [`10.4`, `10.4.0`, `latest` (*10.4/Dockerfile*)](https://github.com/gitlab-ce-zh/gitlab-ce-zh/blob/master/10.4/Dockerfile)
...
[Docker Hub Stars]: https://img.shields.io/docker/stars/gitclub/gitlab-ce-zh.svg
[Docker Hub Pulls]: https://img.shields.io/docker/pulls/gitclub/gitlab-ce-zh.svg
[Docker Image Layers]: https://images.microbadger.com/badges/image/gitclubg2218/gitlab-ce-zh.svg
[Docker Hub Link]: https://hub.docker.com/r/gitclub/gitlab-ce-zh/

[GitHub Stars]: https://img.shields.io/github/stars/gitlab-ce-zh/gitlab-ce-zh.svg?style=social&label=Stars
[GitHub Link]: https://github.com/gitlab-ce-zh/gitlab-ce-zh/

[License]: https://img.shields.io/github/license/twang2218/gitlab-ce-zh.svg
[License Link]: https://github.com/gitlab-ce-zh/gitlab-ce-zh/blob/master/LICENSE.md

[Deploy to Docker Cloud]: https://img.shields.io/badge/deploy-docker_cloud-orange.svg?style=flat
[Docker Cloud Link]: https://cloud.docker.com/stack/deploy/?repo=https://github.com/gitlab-ce-zh/gitlab-ce-zh
...
```bash
docker run -d -p 3000:80 gitclub/gitlab-ce-zh:10.4.0
```

*可以将 `10.4.0` 换成你所需要的版本标签。*
...
services:
    gitlab:
      image: 'gitclub/gitlab-ce-zh:10.4.0'
...
* `testing` 是比较 [xhang 翻译项目](https://gitlab.com/xhang/gitlab) 的 `v10.4.0` 标签和 [`10-4-stable-zh` 分支](https://gitlab.com/xhang/gitlab/tree/10-4-stable-zh) 的差异生成汉化补丁，并基于官方镜像 `gitlab/gitlab-ce:10.4.0-ce.0` 应用汉化结果进行构建的。
...
各个小版本分支的 `Dockerfile`，如 `10.4/Dockerfile` 等，都是使用 `generate` 命令生成的。因此只需要更新 `build-version.sh` 中的相关变量就可以完成各个版本镜像的升级。
...
例如：`./build.sh branch 10.4.0-ce.0 v10.4.0 8-15-stable-zh`

这表明将使用 `gitlab/gitlab-ce:10.4.0-ce.0` 做为基础镜像，并且使用上游版本标签 `v10.4.0` 作为对比的基础标签版本，也就是对应于基础镜像版本的标签，然后使用汉化分支 `8-15-stable-zh` 进行对比，生成汉化补丁，由此构建一个名为 `gitclub/gitlab-ce-zh:8-15-stable-zh` 的镜像。

### `tag` - 构建某个汉化标签的镜像

格式为：`./build.sh tag <基础镜像标签> <英文版本标签>`

例如： `./build.sh tag 10.4.0-ce.0 v10.4.0`

这表明将使用 `gitlab/gitlab-ce:10.4.0-ce.0` 镜像为基础镜像，以 `v10.4.0` 为基础对比版本，以 `v10.4.0-zh` 为汉化版本进行对比生成汉化补丁，并构建一个名为 `gitclub/gitlab-ce-zh:10.4.0` 的镜像。

### `run` - 运行某个构建好的镜像

格式为：`./build.sh <镜像标签>`

例如： `./build.sh run 10.4.0`

这将会以命令 `docker run -d -P gitlab-ce-zh/gitlab-ce-zh:10.4.0` 来运行镜像。这里使用的是 `-P`，因此会随机映射端口。方便测试环境测试，避免和其它端口冲突。
